    ...
    for my $sender( @senderliste ) {
        if($sender->{number} > 0) {
            ...
            if(defined $logo) {
                ...
                $m3u .= "pipe://" . $ffmpeg . " -loglevel fatal -threads 2 -re -i \"http://" . $hostip . ":" . $port . "/channel?id=" . $sender->{_id} . "\" -fflags +genpts+ignidx+igndts -vcodec copy -acodec copy -copyts -muxdelay 0 -f mpegts -mpegts_copyts 1 -metadata service_name=\"" . $sender->{name} . "\" pipe:1\n";
            }
        }
    }
    return $m3u;
