...
my $tempFile;
my $preserveFirstHarmonizeInput = 1;
my $preservedHarmonizeInputPath = '';
...
sub streamReencodedWindow {
    ...
    my $tmpdir;
    if ($preserveFirstHarmonizeInput && !$preservedHarmonizeInputPath) {
        $tmpdir = tempdir('plutotv-harmonize-XXXXXX', TMPDIR => 1, CLEANUP => 0);
    } else {
        $tmpdir = tempdir('plutotv-harmonize-XXXXXX', TMPDIR => 1, CLEANUP => 1);
    }
    ...
    buildLocalWindowPlaylistFile($window->{audioSegments}, $audioPlaylist) or return 0 if $hasAudio;

    appendRecentLog('DEBUG Lokale Video-Playlist fertig fuer ' . $channelId . ': ' . $videoPlaylist . ' bytes=' . ((-s $videoPlaylist) || 0));
    print STDERR "[HARMONIZE][$channelId] video playlist ready: $videoPlaylist (" . ((-s $videoPlaylist) || 0) . " bytes)
";
    if ($hasAudio) {
        appendRecentLog('DEBUG Lokale Audio-Playlist fertig fuer ' . $channelId . ': ' . $audioPlaylist . ' bytes=' . ((-s $audioPlaylist) || 0));
        print STDERR "[HARMONIZE][$channelId] audio playlist ready: $audioPlaylist (" . ((-s $audioPlaylist) || 0) . " bytes)
";
    }
    if ($preserveFirstHarmonizeInput && !$preservedHarmonizeInputPath) {
        $preservedHarmonizeInputPath = $videoPlaylist;
        appendRecentLog('Erste Harmonize-FFmpeg-Playlist bleibt erhalten fuer manuellen Test: ' . $videoPlaylist);
        print STDERR "[HARMONIZE][$channelId] preserved first ffmpeg playlist: $videoPlaylist
";
    }
...
");
    ...
    for my $encoder (@encoders) {
        ...
        appendRecentLog('DEBUG Harmonize ffmpeg cmd [' . $encoder . '] ' . $cmdline);
        print STDERR "[FFMPEG][$channelId][$encoder] CMD $cmdline
";

        my $err = gensym();
        my $ffpid = eval { open3(undef, my $ffh, $err, @cmd) };
        if (!$ffpid) {
            appendRecentLog('ffmpeg-Start fehlgeschlagen [' . $encoder . ']: ' . $channelId . ' | ' . ($@ || $!));
            print STDERR "[FFMPEG][$channelId][$encoder] start failed: " . ($@ || $!) . "
";
            next ENCODER;
        }
        binmode($ffh);
        ...
        while (1) {
            ...
            for my $fh (@ready) {
                ...
                if ($read == 0) {
                    ...
                    if ($fh == $ffh) {
                        $stdout_open = 0;
                        appendRecentLog('DEBUG Harmonize EOF von ffmpeg-stdout fuer ' . $channelId . ' nach ' . $stdout_chunks . ' Chunks und ' . $stdout_bytes . ' Bytes');
                        print STDERR "[FFMPEG][$channelId][$encoder] EOF stdout after $stdout_chunks chunks / $stdout_bytes bytes
";
                        close($ffh);
                    } else {
                        $stderr_open = 0;
                        appendRecentLog('DEBUG Harmonize EOF von ffmpeg-stderr fuer ' . $channelId);
                        print STDERR "[FFMPEG][$channelId][$encoder] EOF stderr
";
                        close($err);
                    }
                    next;
...
/, $chunk, -1;
                    for my $line (@lines) {
                        ...
                        print STDERR "[FFMPEG][$channelId][$encoder] $line
";
                    }
                    next;
                }
                ...
                $stdout_bytes += $read;
                if ($stdout_chunks == 1) {
                    appendRecentLog('DEBUG Harmonize erster ffmpeg-Chunk fuer ' . $channelId . ': ' . $read . ' Bytes [' . $encoder . ']');
                }
                $chunk = correctMpegTsTimestamps($chunk, $channelId, ($firstChunk && $window->{startsAfterDiscontinuity}) ? 1 : 0);
                $firstChunk = 0;
                my $ok = eval { $client->write($chunk); $client->flush(); 1 };
                unless ($ok) {
                    $client_alive = 0;
                    print STDERR "[HARMONIZE][$channelId][$encoder] client write failed: $@
";
                    last;
                }
                if ($stdout_chunks == 1) {
                    appendRecentLog('DEBUG Harmonize erster Client-Write fuer ' . $channelId . ': ' . length($chunk) . ' Bytes [' . $encoder . ']');
                }
            }
            last unless $client_alive;

            my $child_done = waitpid($ffpid, WNOHANG);
            if (defined $child_done && $child_done == $ffpid && !$stdout_open && !$stderr_open) {
                last;
            }
            ...
            if (!$has_output) {
                if (time() - $started_at >= $startup_timeout) {
                    ...
                    print STDERR "[HARMONIZE][$channelId][$encoder] startup timeout after ${startup_timeout}s without output
";
            ...
            if (time() - $last_output_at >= $stall_timeout) {
                ...
                print STDERR "[HARMONIZE][$channelId][$encoder] stall timeout after ${stall_timeout}s
";
                $aborted_for_timeout = 1;
                kill 'TERM', $ffpid;
                last;
        ...
        my $exit = $? >> 8;
        my $signal = $? & 127;
        appendRecentLog('DEBUG Harmonize ffmpeg beendet fuer ' . $channelId . ' [' . $encoder . '] exit=' . $exit . ' signal=' . $signal . ' read=' . $stdout_bytes . ' written=' . $stdout_bytes);
        print STDERR "[FFMPEG][$channelId][$encoder] exit=$exit signal=$signal read=$stdout_bytes bytes
";
        ...
        if (!$has_output && $encoder ne 'libx264') {
            appendRecentLog('ffmpeg ohne Output [' . $encoder . '], Fallback auf libx264: ' . $channelId);
            next ENCODER;
        }
